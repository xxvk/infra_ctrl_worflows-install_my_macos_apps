---
component_id: "davinci-resolve"
name: "DaVinci Resolve"
category: "Video"
tier: "optional"
lifecycle_status: "active"
source: "app_store"
delivery_method: "app-store"
brew_cask: null
brew_formula: null
official_url: "https://www.blackmagicdesign.com/products/davinciresolve"
check_command: null
install_after: []
permissions_required: []
secrets_policy: "Never store passwords, API keys, recovery codes, or license secrets here."
installed_measurement_method: "local_du"
---
# DaVinci Resolve

## Delivery

- Preferred source: Mac App Store: https://apps.apple.com/us/app/davinci-resolve/id571213070?mt=12
- This is an Optional item; do not install automatically during a Core deployment.
- App Store installs require the user to complete Get/Download, password, Touch ID, or 2FA.

## Size tracking

- Planning download estimate: 5000000000 bytes (`size_gb` catalog estimate).
- Installed footprint above is measured locally; download bytes remain `null` unless a package transfer log is available.
- After installation, record `download_bytes`, `installed_bytes`, `installed_version`, and `installed_at`.

## Post-install checklist

- [ ] Choose App Store or Blackmagic edition deliberately
- [ ] Configure cache/media locations

## Notes

The Mac App Store edition has feature/plugin/control-panel limitations versus the Blackmagic website edition.

## Local evidence (2026-07-16)

- Installed path: `/Applications/DaVinci Resolve.app`
- Installed version: `20.3.2`
- Installed footprint: `3084353536` bytes, measured with `du -sk`.
- Download bytes are recorded separately; a local bundle footprint is not treated as download volume.
